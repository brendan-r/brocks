...
guess_db_connection <- function (db_con_classes = "PostgreSQLConnection") {
  ...
  good_advice <- "Better to specify a database connection explicitly."
  ...
  if(sum(is_con) > 1)
    stop("There is more than one active database connections in memory",
         good_advice)
...
#' Retreive a database query from a SQL statement in a text file
#'
#' A convenience function which wraps \code{\link{dbGetQuery}}, by default
#' providing some local caching
#'
#' @param sql_file A path to a file to SQL
...
#' @export
get_query <- function (
  sql_file,  con = guess_db_connection(),  local_cache = TRUE,
  cache_dir = "./.sql_cache",  gitignore = TRUE
) {
  # If there is no sql file, throw an error
